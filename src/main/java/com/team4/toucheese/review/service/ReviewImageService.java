...
import com.team4.toucheese.review.dto.ReviewImageDetailDto;
...
public class ReviewImageService {
    ...
    //리뷰사진 모아보기
    public Page<ReviewImageDetailDto> findAllReviewImage(Long studioId, Pageable pageable, Long menuId){
        ...
        //DTO 변환
        List<ReviewImageDetailDto> reviewImageDtos = reviewImages.stream().map(ReviewImageDetailDto::fromEntity).toList();

        //페이징
        int start = (int) pageable.getOffset();
        int end = Math.min((start + pageable.getPageSize()), reviewImageDtos.size());
        List<ReviewImageDetailDto> pagedReviewImageDtos = reviewImageDtos.subList(start, end);
    ...
    public ReviewImageWithMenu reviewImageWithMenu(Long studioId, Pageable pageable, Long menuId){
        ...
        Page<ReviewImageDetailDto> reviewImageDtos = findAllReviewImage(studioId, pageable, menuId);
